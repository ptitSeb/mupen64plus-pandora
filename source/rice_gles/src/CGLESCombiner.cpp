...
#include "CGLESCombiner.h"

CGLESCombiner::CGLESCombiner()
{
    m_lastIndexGLES = 0;
}
...
int CGLESCombiner::FindCompiledMux( )
{
    for( uint32 i=0; i<m_vCompiledGLESSettings.size(); i++ )
    {
        if( m_vCompiledGLESSettings[i].dwMux0 == (*m_ppDecodedMux)->m_dwMux0 && m_vCompiledGLESSettings[i].dwMux1 == (*m_ppDecodedMux)->m_dwMux1 )
        {
            m_lastIndexGLES = i;
            return i;
        }
...
int CGLESCombiner::ParseDecodedMux()
{
    GLESCombinerSaveType res;
    res.numOfUnits = 2;
    ...
    for( int i=0; i<res.numOfUnits*2; i++ ) // Set combiner for each texture unit
    {
        ...
        GLESCombinerType &unit = res.units[i/2];
        GLESCombType &comb = unit.Combs[i%2];
